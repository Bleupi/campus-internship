...
- `apps/web`: `/signup` and `/login` screens (MUI, ADR-0017 theme, `react-hook-form` + the shared Zod schemas), a guarded `/dashboard` placeholder route with a working logout button, and a 401-retry-once interceptor in the API client that transparently refreshes an expired access token instead of surfacing a logout.

Code-review follow-up: a global `PrismaExceptionFilter` now translates Prisma errors (P2002/P2025/P2003) into proper HTTP responses instead of letting them fall through to a generic 500.
