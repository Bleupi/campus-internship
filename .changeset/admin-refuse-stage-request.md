...
- `apps/web`: a "Refuser" action per row of "Demandes à traiter", disabled (with a hint) without a referent; a dialog with three recurring checkable reasons plus a free "Précision" field, built client-side into the single `reason` string the API expects. "Il manque les informations suivantes :" is a static hint above Précision, not a selectable reason and not its own input (QA feedback) — whatever is missing is written into Précision like any other detail. A 409 shows a reload toast and the row is dropped from the list on either outcome.
