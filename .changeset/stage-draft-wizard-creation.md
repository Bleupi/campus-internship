...
- The inline "create a new organism"/"create a new tutor" options in their respective dropdowns read "Créer un nouvel Organisme"/"Créer un nouveau Tuteur" and are visually set apart (icon, bold, accent color) from real results; the inline tutor-creation form also exposes the "Accepte d'être contacté par téléphone" switch (`acceptsPhoneContact`).
