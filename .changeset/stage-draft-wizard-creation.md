...
- Seeds the fixed `OrganismStructureType` label list used by the inline-organism-creation dropdown: Secteur Médico-social, Sanitaire, Fédérale, Libéral and Associatif (the dev seed removes any other label left over from an earlier list).
...
- When the inline organism or tutor can't be created while saving a draft, the API answers 500 with a specific message ("Impossible de créer l'organisme/le tuteur. Le brouillon n'a pas été enregistré.") and the whole transaction rolls back, so neither the stage nor a half-created organism is persisted; the wizard keeps showing its generic save error.
