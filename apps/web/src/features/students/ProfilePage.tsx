...
export function ProfilePage() {
  ...
  // Issue #45: nudge, not a verifiable guarantee — client-side only, never persisted.
  ...
  const submitCertificate = (file: File) => {
    ...
    uploadInsuranceCertificate.mutate(file, {
      ...
      // student to re-tick the checkbox just to retry the same file (issue #45 review).
  ...
  const cancelPendingAction = () => {
    ...
    // over to a different file picked afterwards (issue #45 review).
