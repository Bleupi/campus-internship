...
describe("Stage management feature flag off (e2e)", () => {
  ...
  it("PATCH /admin/stage-requests/:id/refuse: 404 (module never registered), not 401 (issue #151)", async () => {
    await request(app.getHttpServer())
      .patch("/admin/stage-requests/some-id/refuse")
      .send({ version: 0, reason: "Motif" })
      .expect(404);
  });

  it("PATCH /admin/stage-requests/:id/validate: 404 (module never registered), not 401 (issue #152)", async () => {
    await request(app.getHttpServer())
      .patch("/admin/stage-requests/some-id/validate")
      .send({ version: 0 })
      .expect(404);
  });
  ...
  it("POST /admin/referents: 404 (module never registered), not 401 (issue #150)", async () => {
    await request(app.getHttpServer()).post("/admin/referents").send({}).expect(404);
  });
});
